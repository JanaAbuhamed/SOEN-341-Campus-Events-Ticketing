...
def organizer_analytics(request):
    ...
    overall_attendance_rate = (
        (total_attendees / total_capacity * 100) if total_capacity > 0 else 0
    )
    ...
    for event in approved_events:
        ...
        attendance_rate = (
            (attendees_count / event.capacity * 100) if event.capacity > 0 else 0
        )
        saves_count = event.saved_by.count()  # Using the related_name from SavedEvent

        # Engagement metrics
        save_to_attendance_ratio = (
            (saves_count / attendees_count * 100) if attendees_count > 0 else 0
        )
        engagement_score = (
            (attendees_count + saves_count) / event.capacity * 100
            if event.capacity > 0
            else 0
        )
...
def export_attendees_csv(request, event_id):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="{event.title}_attendees.csv"'
    )
...
def participation_trend(request):
    data = (
        SavedEvent.objects.annotate(date=TruncDate("created_at"))
        .values("date")
...
def claim_trend_by_payment(request):
    ...
    qs = (
        Payment.objects.annotate(date=TruncDate("created_at"))  # or 'timestamp' field
        .values("date")
        .annotate(count=Count("id"))
        .order_by("date")
    )
